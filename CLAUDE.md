...
- **The mobile Calendar is one chrome, five bodies, two axes** (D-122). `MobileCalendar` is
 ...
 five at `?horizon` (`mobile/CalendarHarness.tsx`). The Year's marks (shade + the
 day numeral that indexes it) are two layouts over `calendar/YearParts.tsx` —
 verify both shells at `?year` (`calendar/YearHarness.tsx`). D-106, D-127.
