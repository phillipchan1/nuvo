...
| Drag-to-reschedule | ✅ | `CalendarPane.tsx` `eventDrop={onDrop}`; `editable` per-event (`writable` for Google/iCloud, including all-day); timed → all-day refused by `allowAnytimeLanding` / `eventAllow` | — | — |
| Drag-to-resize duration | ✅ | `eventResize={onResize}`; `eventResizableFromStart`; timed snap 15m; all-day chips resize from left/right handles (daygrid) | — | — |
...
| Multi-day (spanning) events | ⚠️ | `DraftComposer.tsx` (`multiDay`), `lib/dates.ts` `allDayRangeFromStart` / `spanFromCalendarDrop`; desktop all-day chips drag across days and resize from either edge | Multi-day works for **all-day** spans only. A multi-day **timed** event is not composable in the UI. | low |
