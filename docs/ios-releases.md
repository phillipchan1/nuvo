...
| Widget opens the capture / chat sheet but you still have to tap the field | The sheet is up and the caret is not. SPA: `useRaiseKeyboard` (immediate + 120ms + linger to 1.5s, no steal once a chip is focused). Native: walking `_setKeyboardDisplayRequiresUserAction:` at `load(webview:)` is often a no-op — `WKContentView` is not in the tree yet. The load-bearing half is the WKContentView assist swizzle in `NuvoWatchPlugin` (D-115 follow-up, 2026-08-25). Needs a new TestFlight; the JS half alone cannot raise keys. Reproduce with `xcrun simctl openurl booted nuvo://capture` |
| TestFlight app flashes and dies / "Fatal error" / `NSUnknownKeyException` for `keyboardDisplayRequiresUserAction` | D-115's first setter fell through to `setValue(_:forKey:)` when WKWebView did not respond to the private selector (iOS 13+ — the setter is on `WKContentView`). That exception kills the process in `load(webview:)`, so the SPA never mounts. The setter must `responds(to:)`-guard and walk the view tree; never KVC this key. |
