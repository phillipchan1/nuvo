...
| **Trash** | `tasks.status = 'trashed'`, `tasks.trashed_at` | Where a deleted task rests until a human empties it. An icon on the rail's tab strip and on the phone's Tasks control — not a peer of Today and Inbox, shown only when it holds something, not a drop destination (D-136). **Restore** returns a task to the undated funnel place (Inbox, or its project's backlog) and takes you there; **Delete forever** is the one act with no undo (D-104). |
| **Reminder** | `user_settings.reminder_prefs` (defaults), `reminders` (overrides) | The one time Nuvo speaks first, and only about a named commitment: a meeting starting, a block you scheduled starting, a deadline arriving — including all-day events, and more than one lead on the same item. Never a planning nudge. Off until asked (D-102, D-138). |
| **Lead** | `reminders.leads` (`integer[]`) | How long before a commitment a reminder speaks — "10 minutes before", "1 day before". An item may have several (cap 5). Empty list silences that one item, which is a different thing from having no preference (no row, follow Settings). |
...
| **Domain mark** | `domains.icon`, `domainMark.ts`, `DomainMark.tsx` | One scalable identity for a domain. Its user-chosen vector symbol and color appear everywhere; on the wall and open domain, thirteen weeks of presence form a generated halo around that same center. Native emoji and the separate four-form sigil chooser were retired in D-135. It is not an altitude glyph. |
| **Counts toward** | Event inspectors, `eventDomainAttribution` | The visible answer to which domain receives a countable event's time. Calendar says where the event lives; Counts toward says where its actuals land. A person's per-event correction outranks both the calendar default and AI inference (D-142). |
...
| **Sign in with email** | `signInWithPassword` on Login | The App Review door on native iOS (and `?review` / `VITE_REVIEW_LOGIN`): email + password, no OTP. Apple and Google stay on the same screen (Guideline 4.8). An email **code** remains a secondary in-window path. Credentials live in App Store Connect Review Information, never in the repo. D-137 · [`app-store-review.md`](../app-store-review.md). |
...
| **No time yet** · **On the clock** | The two groups in the Today list, on both screens: today's work that has no time on the calendar, then the work that does, in clock order. Rows don't show a clock, so the group is how you can tell which part of today is still only a wish (D-148). The day-level form of *loose* / *has a time*. |
...
| **Capture** (the door) | The one door for anything you just thought of, on every shell: the phone's floating ＋ (a sheet), **⌘K** in the desktop app and **⌥Space** over any Mac app (a panel) — one body, `CaptureDoor`. A single free-text line, parsed, plus a **Task · Event · Slot** switch. Task is the default and the common answer — a scheduled task *is* a time block, so a time in the sentence already lands on the calendar. **Event** is for the thing that has to exist on your *external* calendar: guests, a Meet link, something the people you share a calendar with must see. **Slot** is a block of time that will hold several things — it always has a clock, and `@domain` gives it its affinity. ⌘K is capture only: no search, commands or chat (N-19). Say "quick add" for the desktop door if you must; it is the same thing. D-125, D-149. |
...
| **Year** (Schedule view · both shells) | The Schedule's furthest-out lens: twelve month grids of day numerals — a **map of the year**, today marked with the signal ring, a tap to drill into a day (desk) or a month (phone). On the desk the months **fill the pane** (equal row fractions; each month stretches the weeks it has) and the year numeral sits once in the Schedule toolbar (on the phone, once in the top-bar hero). Load shading lived here and was cut (D-128): it competed with the dates and answered a question the chat's `read_calendar_load` already owns. Desktop `Y`; on the phone it is the **Y** rung of the horizon ladder. D-127, D-128, D-129. |
...
| Term | Meaning |
|---|---|
| **Rollover** | 00:05 America/Los_Angeles via pg_cron. Rolled tasks: `do_date = today`, `start_time` cleared (lands in **anytime**), duration kept, `roll_count` + days carried (↻Nd, D-147), mirror event deleted, ↻ badge. A recurring occurrence rolls only until the series' next occurrence comes due (so a daily one never does). D-141, D-145. |
