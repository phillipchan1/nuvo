...
| **Year** (Schedule view · both shells) | The Schedule's furthest-out lens: twelve month grids of day numerals — a **map of the year**, today marked with the signal ring, a tap to drill into a day (desk) or a month (phone). On the desk the months **fill the pane** (equal row fractions; each month stretches the weeks it has) and the year numeral sits once in the Schedule toolbar. Load shading lived here and was cut (D-128): it competed with the dates and answered a question the chat's `read_calendar_load` already owns. Desktop `Y`; on the phone it is the **Y** rung of the horizon ladder. D-127, D-128, D-129. |
