...
| **Trash** | `tasks.status = 'trashed'`, `tasks.trashed_at` | Where a deleted task rests until a human empties it. A quiet icon on the rail / Tasks strip when it holds something — never a peer of Today or Inbox. **Restore** returns a task to the undated funnel place (Inbox, or its project's backlog) and takes you there; **Delete forever** is the one act with no undo (D-104). |
...
| **Year** (Schedule view · both shells) | The Schedule's furthest-out lens: twelve month grids where each day is **shaded by how loaded it is**, not labelled by date. Five bands — **nothing on · light · busy · full · overcommitted** — from `dayLoad` in the shared day-shape kernel, so the desk, the phone and the chat weigh the same Tuesday identically. Answers *where is this heavy, and where is there nothing* — the day-altitude twin of what **On Deck** answers for projects across weeks. The headline names the **longest clear run ahead** and its dates, never a count of free days: forty scattered free Tuesdays and one free fortnight are the same number and only one is somewhere a week of work fits. Desktop `Y` / ⌘K; on the phone it is the **Y** rung of the horizon ladder. A day with nothing on it is bare paper, never a tint. D-106. |
