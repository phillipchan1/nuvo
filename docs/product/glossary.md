...
| **Grooming** (a domain) | The *routing* axis, the other half of **Kept** — can Nuvo file a capture, a project or a meeting into this domain without asking? A domain reads *needs grooming* when it has only a name, *groom to finish* when you've written its charter but not expanded it, *needs detail* when the expansion found nothing specific, **routes loosely** when it has something but not enough to tell it from a neighbour, and *groomed* ("routes clean") when it does. Scored on the signals the routers actually read — never on the raw field count, which is how the mark once claimed *clear* on keywords no router serialized. [`domain-routing.md`](../domain-routing.md). D-088. |
