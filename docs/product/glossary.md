...
| **Code** *(billing)* | An operator's personal share word — a Stripe Promotion Code on the shared friend Coupon (`subscriptions.referral_code`). Friends get 50% off their first invoice; the sharer gets a free month when they pay (up to six). **Not** an invite (calendar guests), **not** an affiliate program (N-17). Settings → Billing · `?code=` links (D-113). | — |
