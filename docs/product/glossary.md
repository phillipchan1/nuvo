...
| **Year** (Schedule view · both shells) | The Schedule's furthest-out lens: twelve month grids where each day is **shaded by how loaded it is**. Five bands — **nothing on · light · busy · full · overcommitted** — from `dayLoad` in the shared day-shape kernel, so the desk, the phone and the chat weigh the same Tuesday identically. Answers *where is this heavy, and where is there nothing* — the day-altitude twin of what **On Deck** answers for projects across weeks. Each cell carries a small day-of-month numeral as the **index** of that shade, not as a second question (D-127): a heatmap you cannot name is a picture. The headline names the **longest clear run ahead** and its dates, never a count of free days: forty scattered free Tuesdays and one free fortnight are the same number and only one is somewhere a week of work fits. Desktop `Y` / ⌘K; on the phone it is the **Y** rung of the horizon ladder. A day with nothing on it is bare paper, never a tint. D-106, D-127. |
