...
/** Chip-style guest input with fuzzy contact autocomplete sourced from synced
 *  event attendees. No extra OAuth scopes needed. */
export function GuestsInput({
  ...
  const add = (email: string, name?: string) => {
    ...
    // Remember display name for chip rendering.
  ...
  const commit = () => {
    const activeSuggestion = open && suggestions[activeIdx];
    if (activeSuggestion) {
      add(activeSuggestion.email, activeSuggestion.display_name);
  ...
  const chipName = (email: string) => nameCache.current[email] ?? email;

  // Raw email suggestion when input looks like an email but no matches yet.
  const rawEmailSuggestion = input.trim() && isValidEmail(input.trim()) && !suggestions.some((s) => s.email === input.trim().toLowerCase());
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map((email) => (
            ...
            >
              <Avatar email={email} name={chipName(email)} size="sm" />
              <span className="max-w-[140px] truncate">{chipLabel(email)}</span>
              <button
                type="button"
                onClick={() => remove(email)}
                className="fast -ml-0.5 leading-none text-accent/50 hover:text-accent"
      ...
      <div className="relative">
        <div className="flex items-center gap-2 rounded-lg border border-line bg-surface-2 px-3 py-2 focus-within:border-accent">
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "ArrowDown") { e.preventDefault(); setActiveIdx((i) => Math.min(i + 1, suggestions.length - (rawEmailSuggestion ? 0 : 1))); }
        ...
        {open && (suggestions.length > 0 || rawEmailSuggestion) && (
          <ul className="absolute left-0 right-0 top-full z-50 mt-1.5 overflow-hidden rounded-lg border border-line bg-surface shadow-[var(--shadow-3)]">
            {suggestions.map((c, i) => (
              <li key={c.email}>
                <button
                  type="button"
                  onMouseDown={(e) => { e.preventDefault(); add(c.email, c.display_name); }}
                  className={`fast flex w-full items-center gap-3 px-3 py-2.5 text-left ${
                    i === activeIdx ? "bg-accent/8" : "hover:bg-bg"
                  }`}
                >
                  <Avatar email={c.email} name={c.display_name} size="md" />
                  <div className="flex min-w-0 flex-col">
                    <span className={`text-body font-medium leading-tight ${i === activeIdx ? "text-accent" : "text-ink"}`}>
            ...
            {rawEmailSuggestion && (
              <li>
                <button
                  ...
                  className={`fast flex w-full items-center gap-3 px-3 py-2.5 text-left ${
                    activeIdx === suggestions.length ? "bg-accent/8" : "hover:bg-bg"
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-body text-ink">Invite <span className="font-medium">{input.trim()}</span></span>
                    <span className="text-meta text-muted">Send calendar invite</span>
                  </div>
                </button>
              </li>
            )}
