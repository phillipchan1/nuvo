    ...
    <div className="atmosphere flex h-full items-center justify-center">
      <div className="moment elev-3 w-80 rounded-lg border border-line bg-surface p-7">
        <div className="mb-5 flex items-center gap-2.5">
          <TwilightMark />
          <span className="wordmark wordmark-grad text-[22px] leading-none">Nuvo</span>
        </div>
        <div className="mb-6 text-[12px] leading-relaxed text-muted">Your day, on one surface.</div>
        {!supabaseConfigured && (
          <div className="mb-4 rounded-md border border-signal bg-signal-soft p-2 text-[12px] text-signal">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={busy}
            className="fast w-full rounded-md border border-accent bg-accent py-2 text-[13px] font-medium text-white shadow-sm hover:brightness-110 hover:shadow-[0_8px_20px_-6px_var(--accent-glow)] active:translate-y-px disabled:opacity-50"
          >
            {mode === "signin" ? "Sign in" : "Create account"}
          </button>
        </form>
        <button
          className="fast mt-4 text-[12px] text-muted hover:text-ink"
