// The persistent initiative-readiness strip — the sibling of ProjectReadinessStrip,
// pinned above every initiative view (On Deck · Groom · Table). "How ready are the
// bets to be worked?" is the same answer regardless of which face you're on. Reads the
// SAME lens axes the deck/Groom cards route by (lib/lenses.ts, "initiative"), so the
// roll-up and the per-card grooming meters can never disagree. Grooming is the action,
// so off the Groom view the strip offers a jump into it.
...
export default function InitiativeReadinessStrip({ onGroom }: { onGroom?: () => void }) {
  const { data } = useVertical();
  const now = useMemo(() => new Date(), []);

  const inflight = data.initiatives.filter((i) => isProjectInFlight(i.status));
  const needy = inflight.filter((i) => lensGaps(data, "initiative", i, now).length > 0);
