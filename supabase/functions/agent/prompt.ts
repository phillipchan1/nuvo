...
**Priorities (big_rocks)** — the stored per-week *verdict* for a slate project (landed / carried). It rides on the project's own slate entry as **weekSlate[].verdict** (null until one is recorded), so there is one place to read a priority and no second list to reconcile. Use these tools:
...
- **update_priority** — edit a stored priority's title, win condition, or initiative/project link. Use priorityId from weekSlate.
...
**weekSlate is the answer to "what are my priorities this week"** — not weekPool. When the user names a priority, match it against weekSlate by project name (fuzzy is fine) and pass that project's id; pass priority_id as well when the slate entry has one. **unattachedPriorities** holds the rare project-less notes on the week; they render nowhere the user plans, so offer to give each one a project.
...
**Beyond the snapshot's window.** The snapshot only reaches about two weeks. Anything further out has a tool, and "I can't see that far" is never the answer — it is the answer that sends the user to their other calendar.
- *Which day was that / when is my next X* → **search_events**.
- *How heavy is a stretch — "is November brutal", "when am I quietest", "where could a solid week of writing actually go", "how does the rest of the year look"* → **read_calendar_load**.

**Room is a RUN, not a tally.** When someone asks where a multi-day piece of work fits, answer with the longest unbroken clear stretch and its dates ("Dec 13–20 is the one real gap"), never with how many free days there are in total. Forty scattered free Tuesdays and one free fortnight are the same number and only one of them is somewhere the work can go. If **longest_clear_run** comes back null, say plainly that there is no clear stretch — do not offer the scattered days as though they were one.
...
- **set_reminder** attaches one or more leads to ONE thing — a task, an event, or a slot.
  lead_minutes is minutes before (0 = at the time), a comma-separated list for several
  alerts on the same item ("1440,10"), or "off" to silence just that item. Custom values
  up to 14 days are allowed. Two reminders on one meeting is one call, not two.
  anchor: "deadline" is tasks only; the default anchor is the start. An untimed task with
  a do-date can still take a start reminder (it speaks at the Settings time of day).
...
**A tool that hands you the choices has already answered your question — spend it.** When a tool result or a tool error carries \`Candidates:\`, an \`unresolved\` list, or context shows two rows the user's words could equally mean, that list IS your reply. Write the options into the **sentence**, by whatever tells them apart — the initiative a project sits under, a person's full name — then repeat them as taps:
...
// ── the thin variant ─────────────────────────────────────────────────────────
//
// The hypothesis under test: with 38 CRUD tools, most of the prompt is dead
// weight and the model can compose intent from the vocabulary alone.
//
// The honest version of that is narrower than "delete the rules". Sorting the
// prompt by WHY each rule is there gives three kinds:
//
//   1. Derivable from the schemas — the model works it out. Safe to drop.
//   2. Arbitrary product policy ("never infer a calendar from the subject",
//      "'just finished a call about X' is not a date"). Nothing to derive it
//      FROM — no model intelligence recovers a choice we made. Dropping these
//      fails silently: the chat stays fluent and starts being wrong.
//   3. Context semantics (weekSlate vs weekPriorities). Cured by fixing the
//      snapshot, not the prompt.
//
// So the cut here is one rule, applied literally: **remove only what the TOOL
// LAYER already states.** Every block below names the tools whose description
// carries the same rule, and a contract test fails if that tool stops existing.
// Nothing is lost — it is stated once instead of twice, and it arrives when the
// tool is in play instead of on every turn.
//
// Both variants are derived from ONE source string. The thin prompt is never a
// second copy to keep in sync — that is the drift this file already exists to
// prevent.

interface ToolCarriedBlock {
  id: string;
  /** Tools whose description already carries this rule. */
  carriedBy: string[];
  /** Unique prefix of the block's first line. */
  from: string;
  /** Unique tail of the block's last line. */
  to: string;
  /** Optional one-line pointer left in its place. */
  replaceWith?: string;
}

export const TOOL_CARRIED_BLOCKS: ToolCarriedBlock[] = [
  {
    id: "slot-vs-scheduled-task",
    carriedBy: ["create_slot", "add_to_slot", "schedule_task"],
    from: "**When to make a slot instead of N scheduled tasks",
    to: "rather than defaulting to an hour and quietly truncating.",
  },
  {
    id: "move-vs-create-event",
    carriedBy: ["move_event", "create_calendar_event"],
    from: "**move_event vs create_calendar_event — do not duplicate:**",
    to: "(pass calendar_name if they named one).",
  },
  {
    id: "which-calendar",
    carriedBy: ["create_calendar_event"],
    from: "**Which calendar — the user's call, never yours.**",
    to: "pass the name and it will resolve.",
  },
  {
    id: "priority-tool-list",
    carriedBy: ["create_priority", "update_priority", "complete_priority", "delete_priority"],
    from: "- **create_priority** — bring a project onto this week's slate",
    to: "Pass project_id when there's no stored id.",
  },
  {
    id: "local-time-format",
    carriedBy: ["create_calendar_event", "create_slot", "schedule_task", "create_task"],
    from: "**Building start_time / start_local / end_local for tools:**",
    to: '- Friday at 2 PM → "2026-07-03T14:00"',
    replaceWith:
      "**Times for tools are the user's LOCAL time** — every tool's schema states the exact format it wants.",
  },
  {
    id: "calendar-event-verbs",
    carriedBy: ["move_event", "cancel_event", "decline_event"],
    from: "- move_event re-homes an event onto another writable calendar",
    to: "decline_event marks them as not attending; event stays on others' calendars.",
  },
  {
    id: "marquee",
    carriedBy: ["point_at"],
    from: "You can drive the user's screen — the canvas to the left of this chat.",
    to: "one clean move, not a flicker.",
    replaceWith:
      "You can drive the user's screen with **point_at** — its description lists the destinations and when to use them. Then answer normally; the highlight reinforces your words, it never replaces them.",
  },
];

function cutBlock(src: string, b: ToolCarriedBlock): string {
  const start = src.indexOf(b.from);
  if (start < 0) return src;
  const endAt = src.indexOf(b.to, start);
  if (endAt < 0) return src;
  const end = endAt + b.to.length;
  return src.slice(0, start) + (b.replaceWith ?? "") + src.slice(end);
}

/** The prompt minus every rule the tool layer already states. Cutting a block
 *  leaves the blank line that surrounded it, so the seams are closed at the end
 *  rather than guessed at per-block. */
export const THIN_SYSTEM_PROMPT = TOOL_CARRIED_BLOCKS
  .reduce(cutBlock, STATIC_SYSTEM_PROMPT)
  .replace(/\n{3,}/g, "\n\n")
  .replace(/\n---\n\n---\n/g, "\n---\n");

export type PromptVariant = "full" | "thin";

export function systemPromptFor(variant: PromptVariant): string {
  return variant === "thin" ? THIN_SYSTEM_PROMPT : STATIC_SYSTEM_PROMPT;
}

/** Only "thin" opts in; anything else (unset, junk) keeps what ships today. */
export function readPromptVariant(raw: string | undefined): PromptVariant {
  return raw?.trim().toLowerCase() === "thin" ? "thin" : "full";
}
